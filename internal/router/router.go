...
func New(db *gorm.DB, cfg config.Config) *gin.Engine {
	...
	engine := gin.New()
	engine.Use(middleware.RequestID(), middleware.ErrorHandler(), middleware.Recovery(), middleware.CORS(cfg.CORSOrigin), middleware.Audit())
	wired := wire(db, cfg)
	engine.GET("/healthz", wired.system.Health)
	engine.GET("/readyz", wired.system.Ready)
	api := engine.Group("/api/v1")
	api.Use(middleware.RateLimit(cfg.RateLimitPerMinute, "global"))
